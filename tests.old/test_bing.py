...
def test_bing_batch_forward():
    ...
    """
    url_submission = (
        "http://spatial.virtualearth.net/REST/v1/Dataflows/Geocode?input=csv&key=test"
    )
    url_check = "http://spatial.virtualearth.net/REST/v1/Dataflows/Geocode/3bf1b729dddd498e9df45515cdb36130"  # noqa
    url_result = "http://spatial.virtualearth.net/REST/v1/Dataflows/Geocode/3bf1b729dddd498e9df45515cdb36130/output/succeeded"  # noqa
    ...
    with requests_mock.Mocker() as mocker, open(
        submission_file, "rb"
    ) as submission_result, open(confirmation_file, "rb") as confirmation_result, open(
        data_file, "rb"
    ) as batch_result:
