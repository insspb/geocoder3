...
def geonames_response(request):
    url = "http://api.geonames.org/searchJSON?q=Ottawa%2C+Ontario&fuzzy=1.0&username=mock&maxRows=1"  # noqa
...
def paid_geonames_response(request):
    url = "http://ws.geonames.org/searchJSON?q=Ottawa%2C+Ontario&fuzzy=1.0&username=mock&maxRows=1"  # noqa
    data_file = "tests/results/geonames.json"
    with requests_mock.Mocker() as mocker, open(data_file, "r") as input:
        mocker.get(url, text=input.read())
...
def test_extra():
    url = "http://api.geonames.org/searchJSON?q=Ottawa%2C+Ontario&fuzzy=1.0&username=mock&maxRows=1&featureClass=A"  # noqa
...
def test_geocoding_with_proximity():
    ...
    url = "http://api.geonames.org/searchJSON?q=Ottawa%2C+Ontario&fuzzy=1.0&username=mock&maxRows=1&east=-75.2465979&west=-76.3539158&north=45.5375801&south=44.962733"  # noqa
